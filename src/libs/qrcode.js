/**
 * http://www.qrcode.com/zh/index.html
...
QRCodeModel.prototype = {
  ...
  createMovieClip: function (targetMc, instanceName, depth) {
    let qrMc = targetMc.createEmptyMovieClip(instanceName, depth);
    let cs = 1;
...
let QRUtil = {
  ...
  getMask: function (maskPattern, i, j) {
    ...
    default:
      throw new Error('不存在的maskPattern:' + maskPattern);
    }
  },
  ...
  getLengthInBits: function (mode, type) {
    if (1 <= type && type < 10) {
      ...
      case QRMode.MODE_KANJI:
        return 8;
      default:
        throw new Error('不存在的QRMode:' + mode);
      }
    } else if (type < 27) {
      switch (mode) {
      case QRMode.MODE_NUMBER:
...
Drawing.prototype.draw = function (oQRCode) {
  ...
  let nWidth = option.width / nCount;
  let nHeight = option.height / nCount;
  ...
  for (let row = 0; row < nCount; row++) {
    for (let col = 0; col < nCount; col++) {
      ...
      ctx.strokeStyle = bIsDark ? option.colorDark : option.colorLight;
      ctx.lineWidth = 1;
      ctx.fillStyle = bIsDark ? option.colorDark : option.colorLight;
...
Drawing.prototype.makeImg = function () {
...
function _getUTF8Length(sText) {
  let replacedText = encodeURIComponent(sText).toString().replace(/%[0-9a-fA-F]{2}/g, 'a');

  return replacedText.length + (replacedText.length != sText ? 3 : 0);
}

export default function createQRCode(option) {
  ...
  option = {
    ...createQRCode.defaultOption,
    ...option
  };
  if (option.text == null) {
    return;
  }
  const oQRCode = new QRCodeModel(_getTypeNumber(option.text, option.correctLevel), option.correctLevel),
    drawing = new Drawing(option);

  oQRCode.addData(option.text);
  oQRCode.make();
  drawing.draw(oQRCode);

  return drawing.makeImg();
}
