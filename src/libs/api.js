  ...
  return fetch(url, params)
    ...
    .then(data => {
      ...
      if (data && data.stat && data.stat.code < 0) {
        return Promise.reject({
          code: data.stat.code,
          logEMsg: { url: `${url}/${payload.data._mt}`, params, res: data },
        });
        ...
        data.stat &&
        data.stat.stateList &&
        data.stat.stateList[0] &&
        data.stat.stateList[0].code !== 0
      ) {
        return Promise.reject({
          code: data.stat.stateList[0].code,
          logEMsg: { url: `${url}/${payload.data._mt}`, params, res: data },
        });
    ...
    .catch(e => {
      ...
      return Promise.reject({
        name: e.name,
        message: e.message,
        logEMsg: { url: `${url}/${payload.data._mt}`, params, res: e.toString() },
      });
    });
}
