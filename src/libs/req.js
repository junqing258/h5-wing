...
function req(payload) {
  ...
  return API(payload, CONFIG.API_URL).then((data) => {
    return data;
  }, (error) => {
    window.sentry && window.sentry.sendLog({
    ...
    // 通用 无网络错误提示
    if (error.name === 'TypeError' && error.message === 'Failed to fetch' || error.message === 'Network request failed') {
